...
| **Save target** | **OPEN — the author is deciding, and has asked for research first.** Project Mutant: a fixed 15, difficulty as a ± modifier on the source — chosen so that 127 save-bonus traits, and the sim's shell and venom prices, are priced against one number. Stone & Spear: the TN set by the threat — 10 weak, 12 standard, 15 strong, 18 extreme — and every one of 188 bestiary behaviours names its own; S&S has no priced save bonuses yet. Crypt Crawler v3 (unaudited): 10 / 13 / 15. **Positions:** Mutant for 15 (its 127 priced traits assume it). Star Crawler and X-Gen price against 15 as *draft positions* — both games are in draft and have decided nothing (mail #17, #18). S&S, having done the research below, now recommends 15. **At the table the two are one roll with the same odds** ("save vs 18" is "save at −3 vs 15"); the difference is what the books print, and whether a save bonus means the same thing in every book. A middle position exists: 15 is the standard, a source may name 10 / 12 / 15 / 18, all bonuses priced against 15 — under which no game changes anything. **The author leans by-threat with a standard anchor** and wants it thought through. **The research (S&S, mail #16):** in B/X and OSE the save number lives on the *character's* table (a level-1 Fighter saves vs poison on 12; a Cleric on 11) and the *source* never sets a target — it names a category and at most adjusts it ("save at −2"). Transposed to ascending, that is exactly `d20 + save bonus vs 15, difficulty as an adjustment on the source`. The per-source TN is the 3e/5e idiom. S&S's four tiers map without loss: weak 10 = +5, standard 12 = +3, strong 15 = 0, extreme 18 = −3; a behaviour reads "CON save at +3" instead of "TN 12", which is the B/X sentence. The referee still grades the threat, as an adjustment, the way B/X referees always have. S&S's re-point is 188 behaviours, four values, one script — not run until the author rules. (mail #2, #6, #7, #8, #9, #16, #17, #18) | open |
| **The check ladder** | **Core, ruled 2026-09-13: 7 easy / 9 ordinary / 11 hard, 9 standard.** The games had differed on what the check *is* — Mutant's is a fallback for what no trait covers, Stone & Spear's was the skill game itself on a 7-standard ladder — not on the arithmetic. The author ruled it for S&S after `skillsim.py` ran 3,000 days of survival checks: at 7 an ordinary person (+0) succeeds 60% and a level-1 band eats 99 days in 100; at 9, 31% and 82 in 100, which is the game S&S's own Chapter 1 describes. A background +1 was worth about +14 points under either ladder, so training did not decide it; the baseline did. S&S 18.20.0 adopted 9; Mutant had it; Star Crawler and X-Gen (both in draft) have no objection. Four games, one ladder. A game may still say what the check is *for*; the numbers are Core. (mail #6, #7, #8, #9) | settled |
